...
- [fabricators design](#fabricators-design)
...
`fabricators` is designed create files from templates.

`fabricators` Templates can be filled via a CLI form or a Grid.

## How is it different from what I already use?

**WORK IN PROGRESS**: `fabricators` is still in the `design` phase.
...
`fabricators` is a part of the [Open Source Universe](https://github.com/intellibus/approach). All Projects in the [Open Source Universe](https://github.com/intellibus/approach) follow the same [Contribution Guidelines & Process](https://github.com/intellibus/approach).
...
`fabricators` only plans to initially support Typescript & JavaScript Applications. However, we would love for the community to contribute versions for other languages (Java, Go, etc.)
